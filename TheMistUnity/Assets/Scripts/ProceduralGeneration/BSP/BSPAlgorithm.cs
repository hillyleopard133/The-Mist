...
public class BSPAlgorithm
{
    ...
    public static BSPNode GetStartingRoom(BSPNode root, int mapWidth, int mapHeight)
    {
        ...
        foreach (var node in leaves)
        {
            ...
            float dx = Mathf.Abs(centerX - targetX);
            float dy = Mathf.Abs(centerY - targetY);

            // Bias heavily toward bottom
            float score = dy * 3f + dx;
